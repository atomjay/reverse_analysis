...
const char *DECRYPTION_KEY = "mysecretkey123";
...
int main() {
    ...
    char temp_filename[] = "/tmp/decrypted_XXXXXX";
    int fd;
    FILE *temp_exe_file;

    // 1. Read encrypted file
    encrypted_file = fopen("projects/06_packing_unpacking/03_custom_encryption_packing/encrypted_program.enc", "rb");
    if (!encrypted_file) {
    ...
    // In a real scenario, you'd use a crypto library like OpenSSL's API
    // For simplicity, we'll use popen to call openssl command line tool
    // This is NOT how you'd do it in a production environment.
    char command[512];
    snprintf(command, sizeof(command), 
             "openssl enc -aes-256-cbc -d -salt -in projects/06_packing_unpacking/03_custom_encryption_packing/encrypted_program.enc -k \"%s\"", 
             DECRYPTION_KEY);
    ...
    // Read decrypted output from pipe
